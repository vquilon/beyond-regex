...
(async function oneko() {
    ...
    // Reemplazo de PopupModal con un modal manual
    ...
    function sleep() {
        ...
        // Aquí se puede adaptar para otra barra de progreso
        const progressBar = document.querySelector(".playback-progressbar");
        if (progressBar) {
            mousePosX = progressBar.getBoundingClientRect().right - 16;
            mousePosY = progressBar.getBoundingClientRect().top - 8;
        }
    }
  
  
    // const nekoSpeed = 10,
      const nekoSpeed = 10 * nekoScale,
      variants = [
        ["classic", "Classic"],
        ["dog", "Dog"],
        ["tora", "Tora"],
    ...
    function create() {
      variant = parseLocalStorage("variant", "conguito");
    ...
    function pickerModal() {
      ...
      function variantButton(variantEnum) {
        ...
        div.style.backgroundImage = `url('/beyond-regex/assets/vendor/oneko/assets/oneko-${variantEnum[0]}.gif')`;
    ...
    while (!document.body) {
      ...
      let keySequence = [];
      let comboTimer;
      const comboTimeout = 1000; // 1 segundo de tiempo máximo entre cada tecla
      const secretCode = ['o', 'n', 'e', 'k', 'o'];
      
      window.addEventListener("keydown", (e) => {
          const key = e.key.toLowerCase(); // Convertir la tecla a minúsculas para evitar problemas con mayúsculas
          ...
          // Iniciar el temporizador para reiniciar el combo si pasa más de 1 segundo
          comboTimer = setTimeout(() => {
              keySequence = []; // Reiniciar el combo si el tiempo entre teclas es mayor a comboTimeout
          }, comboTimeout);
      
          // Verificar si la secuencia de teclas coincide con el combo secreto
          if (keySequence.join(' ') === secretCode.join(' ')) {
              showModal(pickerModal()); // Mostrar el modal cuando se detecta el combo
              keySequence = []; // Reiniciar el combo después de activarlo
              clearTimeout(comboTimer); // Detener el temporizador
          }
      });
