...
function visualize(regexson_tree, regex_flags, canvas_Raphael_paper) {
    ...
    function paintRegex(regexson, regex_flags, canvas_Raph) {
        ...
        var size_item_text = getFontStyles(graph_fontsize, "bold")
            , u = offset_height
            , l = size_item_text.height / 2 + offset_height
            , max_item_width = 0
            , max_item_height = 0;
        ...
        max_item_height = size_item_text.height + 2 * offset_height;
        regex_text_items = canvas_Raph.add(regex_text_items);

        // Establece la altura para el size generado por el texto de la regex
        canvas_Raph.setSize(max_item_width, size_item_text.height + 2 * offset_height);
        ...
        max_item_height = Math.max(raphael_items.height + 3 * offset_height + size_item_text.height, max_item_height);
        ...
        addsOffset(raphael_items.items, offset_height, 2 * offset_height + size_item_text.height - raphael_items.y);
    ...
    function processRegexJSONToRaphaelSVG(regexJSONInfo, offset_x, offset_y) {
        ...
        regexJSONInfo.forEach(function (single_regexjson_info) {
            ...
            // raphael_element = single_regexjson_info.repeat ? generator_Raph_map.repeat(single_regexjson_info, _offset_x, offset_y) : generator_Raph_map[single_regexjson_info.type](single_regexjson_info, _offset_x, offset_y);
            // Agregar un grupo que envuelva a los elementos creados por la funcion
            processed_items_result.push(raphael_element);
            _offset_x += raphael_element.width + _;
    ...
    var generator_Raph_map = {
        ...
        choice: function (regexJSONInfo, offsetX, offsetY) {
            ...
            return a.forEach(function (t) {
                ...
                addsOffset(t.items, n - t.x, h - t.y),
                    d = d.concat(t.items);
                var a = offsetY + h - t.y
                    , o = createChoicesPathItem(regexJSONInfo, offsetX, offsetY, offsetX + 20, a)
                    , p = createChoicesPathItem(regexJSONInfo, f, offsetY, offsetX + i - 20, a);
                d.push(o, p),
                    offsetX + 20 !== n - t.x + t.lineInX && d.push(createLinesPathItem(regexJSONInfo, offsetX + 20, a, n - t.x + t.lineInX)),
                    t.lineOutX + n - t.x != offsetX + i - 20 && d.push(createLinesPathItem(regexJSONInfo, t.lineOutX + n - t.x, a, offsetX + i - 20)),
                    t.x = n,
                    t.y = h,
                    h += t.height + 6
            }),
        ...
        charset: function (regexJSONInfo, offsetX, offsetY) {
            ...
            if (checkCharsetNotEmpty(regexJSONInfo)) {
                var o = createExactCharItem(regexJSONInfo, n[regexJSONInfo.classes[0]], offsetX, offsetY, "Green", "white");
                if (o.rect.r = 5,
            ...
            if (!regexJSONInfo.chars && !regexJSONInfo.ranges.length && !regexJSONInfo.classes.length) {
                var o = createExactCharItem(regexJSONInfo, "AnyChar", offsetX, offsetY, "green", "white");
                return o.rect.r = 5,
                    o
            }
            var g, x, y = [], p = 0, m = 0;
            regexJSONInfo.chars && (g = createExactCharItem(regexJSONInfo, regexJSONInfo.chars, offsetX, offsetY, "LightSkyBlue", "black"),
                g.rect.r = 5,
                y.push(g),
                p = g.width),
                regexJSONInfo.ranges.forEach(function (t) {
                    t = t.split("").join("-");
                    var n = createExactCharItem(regexJSONInfo, t, offsetX, offsetY, "teal", "white");
                    n.rect.r = 5,
                        y.push(n),
                        p = Math.max(n.width, p)
                }),
                regexJSONInfo.classes.forEach(function (t) {
                    var i = createExactCharItem(regexJSONInfo, n[t], offsetX, offsetY, "Green", "white");
                    i.rect.r = 5,
                        y.push(i),
        ...
        group: function (regexJSONInfo, offsetX, offsetY) {
            ...
            var container_group = {
                type: "group",
                class: `g:${regexJSONInfo.type}:${[regexJSONInfo.indices[0] + 1, regexJSONInfo.indices[1] - 1].join(';')}`,
                children: subgroups_items_element.items,
                transform: `t${subgroups_items_element.x},${subgroups_items_element.y}`,
                x: subgroups_items_element.x,
                y: subgroups_items_element.y,
                width: subgroups_items_element.width,
                height: subgroups_items_element.height
            };
            var container_group = createGroupContainerItem(
                regexJSONInfo, subgroups_items_element.items,
                subgroups_items_element.x, subgroups_items_element.y,
            ...
            if (regexJSONInfo.num) {
                ...
                addsOffset(subgroups_items_element.items, 10, 0);
                var rect_width = subgroups_items_element.width + 20;
                var rect_height = subgroups_items_element.height + 20;
                var rect_item = {
                    type: "rect",
                    class: `rect:${regexJSONInfo.type}:${regexJSONInfo.indices.join(';')}`,
                    x: offsetX,
                ...
                var text_item = createTextItemElement(regexJSONInfo, rect_item.x + rect_item.width / 2, rect_item.y - 2, `Group ${regexJSONInfo.name}#${regexJSONInfo.num}`);
                var max_width = Math.max(text_item.width, rect_width);
                var offset_x_width = (max_width - rect_width) / 2;

                var container_item = {
                    type: "group",
                    class: `g:${regexJSONInfo.type}:${regexJSONInfo.indices.join(';')}`,
                    children: [container_group, rect_item, text_item.label],
                    transform: `t${offsetX + offset_x_width},${offsetY}`,
                    x: offsetX + offset_x_width,
                    y: offsetY,
                    width: max_width,
                    height: rect_height + text_item.height + 4
                };

                var all_items = subgroups_items_element.items.concat([rect_item, text_item.label]);
                ...
                return {
                    ...
                    x: offsetX + offset_x_width,
                    y: offsetY,
                    lineInX: offset_x_width + subgroups_items_element.lineInX + 10,
                    lineOutX: offset_x_width + subgroups_items_element.lineOutX + 10
                }
            }
            ...
            return {
                ...
                x: subgroups_items_element.x,
                y: subgroups_items_element.y,
                lineInX: subgroups_items_element.lineInX,
                lineOutX: subgroups_items_element.lineOutX
            }; // items_subgroup
        },
