// (function () {
    ...
    function visualize(regexson_tree, regex_flags, canvas_Raphael_paper) {
        ...
        function paintRegex(regexson, regex_flags, canvas_Raph) {
            ...
            var h = setFontStyles(graph_fontsize, "bold")
                , u = offset_height
                , l = h.height / 2 + offset_height
                , f = 0
                , p = 0;
            f = regex_text_items.reduce(function (t, e) {
                return e.x = t,
                    e.y = l,
                    t + e.text.length * h.width
            }, u),
                f += offset_height,
                p = h.height + 2 * offset_height,
                regex_text_items = canvas_Raph.add(regex_text_items),
                canvas_Raph.setSize(f, h.height + 2 * offset_height);
            var raphael_items = generateRaphaelSVGItems(regexson.tree, 0, 0);
            p = Math.max(raphael_items.height + 3 * offset_height + h.height, p),
                f = Math.max(raphael_items.width + 2 * offset_height, f),
                canvas_Raph.setSize(f, p),
                container_regex_rect.attr("width", f),
                container_regex_rect.attr("height", p),
                addsOffset(raphael_items.items, offset_height, 2 * offset_height + h.height - raphael_items.y),
                        ...
                        case CHARSET_NODE:
                            ...
                            (!i || item_regexson.exclude) && regex_items_text.push(createItemRegexText("[")),
                                ...
                                item_regexson.ranges.forEach(function (t) {
                                    regex_items_text.push(createItemRegexText(g(t[0] + "-" + t[1]), colors_map.charsetRange))
                                }),
                                item_regexson.classes.forEach(function (t) {
                                    regex_items_text.push(createItemRegexText("\\" + t, colors_map.charsetClass))
                                }),
                                regex_items_text.push(createItemRegexText(g(item_regexson.chars), colors_map.charsetChars)),
                                (!i || item_regexson.exclude) && regex_items_text.push(createItemRegexText("]"));
                            break;
                        default:
        ...
        var generator_Raph_map = {
            ...
            group: function (regexJSONInfo, offsetX, offsetY) {
                ...
                if (regexJSONInfo.num) {
                    ...
                    var container_group = {
                        type: "group",
                        class: `g:${regexJSONInfo.type}:${regexJSONInfo.indices.join(';')}:t${offsetX + offset_x_width},${offsetY}`,
                        subs: [container_subgroup, item_rect, item_text.label],
                ...
                return {
                    items: [container_subgroup],
                    width: container_subgroup.width,
                    height: container_subgroup.height,
                    x: item_subgroup.x,
                    y: item_subgroup.y,
